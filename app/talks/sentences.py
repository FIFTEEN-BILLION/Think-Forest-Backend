...
# 도움 요청만 한 답은 길어도 생각·근거로 인정하지 않는다. 뒤에 실제 짐작을
# 덧붙인 '모르겠지만 컵이 차가워서 그런 것 같아'는 fullmatch에 걸리지 않는다.
_HELP_ONLY = re.compile(
    r"(?:(?:나|저)(?:는|도)?\s*|아직\s*|잘\s*|정말\s*|진짜\s*|너무\s*|조금\s*|전혀\s*|왜냐하면\s*|"
    r"아무것도\s*|도저히\s*|무슨\s*말인지\s*|왜\s*그런지\s*|어떻게\s*해야\s*할지\s*)*"
    r"(?:몰라(?:요|서)?|모르겠(?:어|어요|다|는데|는데요)?|모르기\s*때문이(?:야|에요)|"
    r"모르(?:는\s*것\s*같아|는\s*거\s*같아)|모르는데|모르니까|"
    r"어려워(?:요)?|어렵(?:다|네요)|헷갈려(?:요)?|이해(?:가)?\s*(?:안\s*(?:돼|돼요|가|가요)|안돼)|"
    r"글쎄(?:요)?|힌트(?:를)?\s*(?:좀\s*)?(?:줘|줘요|주세요)|"
    r"(?:설명|도움)(?:을)?\s*(?:좀\s*)?(?:해\s*)?(?:줘|줘요|주세요))"
)
...
def check_sentence(text: str) -> SentenceCheck:
    ...
    help_text = re.sub(r"[.!?~…ㅠㅜㅋㅎ\s]+$", "", t)
    if _HELP_ONLY.fullmatch(help_text):
...
EXPAND_LINES: dict[str, str] = {
    ...
    "unsure": "모르는 건 괜찮아. 바로 답하지 않아도 돼. 헷갈리는 말이나 궁금한 부분부터 함께 살펴보자.",
}

STARTERS: dict[str, list[str]] = {
    "default": ["나는 ___라고 생각해.", "왜냐하면 ___ 때문이야.", "예를 들면 ___."],
    "unsure": ["___ 부분이 궁금해.", "___을 쉽게 설명해 줘."],
