...
import httpx, uuid, sqlite3, sys
B="http://127.0.0.1:8000/api/v1"; DB=sys.argv[1]
...
con=sqlite3.connect(DB); con.execute("update children set is_tester=0"); con.commit(); con.close()
def one_turn(label):
    s=c.post(B+"/first-greeting/sessions", headers={**H,"Idempotency-Key":uuid.uuid4().hex}).json()
    r=c.post(B+f"/first-greeting/sessions/{s['sessionId']}/messages", headers=H,
             json={"clientMessageId":uuid.uuid4().hex,"input":{"type":"TEXT","text":"나는 달이라고 불러 줘. 3학년이야"}}).json()
...
c.post(B+"/consents", headers=H, json={"profileId":prof,"items":[{"documentId":ai_doc["id"],"version":ai_doc["version"],"agreed":True}],"actor":"GUARDIAN"})
one_turn("동의 후")
