...
sid = s["sessionId"]
m = c.post(
    B + f"/first-greeting/sessions/{sid}/messages",
    headers=H,
    json={"clientMessageId": uuid.uuid4().hex, "input": {"type": "TEXT", "text": "나는 별이라고 불러 줘. 2학년이야"}},
).json()
...
for t in ["공룡 좋아해", "이빨이 커서 신기해", "질문하는 힘을 키우고 싶어"]:
    c.post(
        B + f"/first-greeting/sessions/{sid}/messages",
        headers=H,
        json={"clientMessageId": uuid.uuid4().hex, "input": {"type": "TEXT", "text": t}},
    )
c.post(
    B + f"/first-greeting/sessions/{sid}/complete",
...
cv = c.post(
    B + "/conversations",
    headers={**H, "Idempotency-Key": uuid.uuid4().hex},
    json={"topicId": "topic_ice_cup", "inputMode": "TEXT", "locale": "ko-KR"},
).json()
cid = cv["conversationId"]
inter = cv.get("nextInteraction") or {}
for t in [
    "놀이터에서 봤어",
    ...
    body["input"] = (
        {"type": "SINGLE_CHOICE", "optionId": inter["options"][0]["id"]}
        if inter.get("type") == "SINGLE_CHOICE" and inter.get("options")
